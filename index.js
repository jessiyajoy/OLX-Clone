...
app.get('/my_products', (req, res) => {
    let sql1 = 'Select * from product_info p where p.sellerID=?';
    let sql2 = 'Select * from order_details o where o.buyer_id=?';
    let sql3 = 'Select * from product_info JOIN order_details ON product_info.id = order_details.product_id where order_details.buyer_id=?';
    let sql4 = 'Select * from feedback JOIN order_details ON feedback.order_id = order_details.order_id where order_details.buyer_id=? ';
    ...
    let query1 = mysqlConnection.query(sql1, currentUser, (err, rows, fields) => {
        ...
        let query1 = mysqlConnection.query(sql2, currentUser, (err, rows, fields) => {
            let query1 = mysqlConnection.query(sql3, currentUser, (err, rows, fields) => {
                let query1 = mysqlConnection.query(sql4, currentUser, (err, rows, fields) => {
                    rating_data = rows;
